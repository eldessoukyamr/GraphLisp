...
// you should add your unit tests on unittests.cpp
...
TEST_CASE("Test graphic types II", "[interpreter]") {
    {
        std::string program = "(rect 10 10 20 20)";
        std::istringstream iss(program);
        Interpreter interp;

        REQUIRE(interp.parse(iss));
        REQUIRE(interp.eval() == Expression(10, 10, 20, 20));
    }
    {
        std::string program = "(fill_rect (rect 1 2 3 4) 255 200 100)";
        std::istringstream iss(program);
        Interpreter interp;

        REQUIRE(interp.parse(iss));
        REQUIRE(interp.eval() == Expression(std::make_tuple(1, 2, 3, 4), 255, 200, 100));
    }
    {
        std::string program = "(ellipse (rect -3 -4 -5 -6))";
        std::istringstream iss(program);
        Interpreter interp;

        REQUIRE(interp.parse(iss));
        REQUIRE(interp.eval() == Expression(std::make_tuple(-3, -4, -5, -6)));
    }
}

TEST_CASE("Test draw command", "[interpreter]") {
    std::string program = "(draw (rect 10 10 20 20))";
    std::istringstream iss(program);
    Interpreter interp;
